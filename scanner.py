...
Changes from v3.9:
- VA, NC, AZ, GA removed from watch list. No credible prevailing wage
  legislative activity found in 2025-2026 sessions. Verified via web search
  June 2026. VA also confirmed exempt via legal counsel (DBA-tied law,
  Honeywell technicians not covered under DBA).
  Removed: VA_LEG, VA_LABOR, NC_LEG, NC_LABOR, AZ_LEG, GA_LEG, GA_LABOR.
  Watch list now: WI, WV only.
- ABC URL corrected to https://www.abc.org/News-Media/News-Releases.

- WI_LEG removed. legis.wisconsin.gov blocks automated cloud access. WI_LABOR retained.
- Total sources: 22 (16 active, 3 watch-list, 3 aggregate).
- Print statement version corrected from v3.8 to v4.0.
"""
...
ALL_SOURCES = JURISDICTIONS + WATCH_LIST + AGGREGATE_SOURCES  # 22 total (16 active + 3 watch-list + 3 aggregate)
...
def main():
    ...
    print(f"PW Monitor Scanner v4.0 — {scan_date}")
